...
class Course {
  constructor(id, acronym, name, section) {
    this.id = id
  ...
  async scrape() {
    this.folders = {}
    this.files = {}
    await this.scrapeFolder(this)
  }

  async scrapeFolder(folder, repeat = 0) {
    try {
      ...
      await newFolders.reduce(async (promise, folder) => {
        await promise
        wait()
        return this.scrapeFolder(folder)
      }, Promise.resolve())
    } catch (err) {
      if (err.response.status === 500 && repeat < 5) {
        ...
        return this.scrapeFolder(folder, repeat + 1)
      }
      const message = `\n
       name: ${folder.name}
       url: ${folder.url}
       repeat: ${repeat}`
      error(err.message + message, "scrapeFolder")
    }
  }
