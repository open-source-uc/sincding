...
class Siding {
  ...
  static async currentCoursesList(session, ignore = []) {
    return await Siding.coursesListFrom(session, urls.currentCoursesURL, ignore)
  }

  static async catalogCoursesListFrom(session, year, term, ignore = []) {
    return await Siding.coursesListFrom(
      session,
      urls.catalogFromURL(year, term),
      ignore
    )
  }
